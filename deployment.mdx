---
title: 7.5. Guía de Despliegue y CI/CD
description: Guía completa para desplegar Retro Game Hub con GitOps y despliegue continuo
icon: rocket
---
...
if [ "$response" != "200" ]; then
    echo "❌ API health test failed: HTTP $response"
    exit 1
fi
...
if [ "$response" != "200" ]; then
    echo "❌ Game service test failed: HTTP $response"
    exit 1
fi
echo "✅ Game service is responding"
...
if [ "$response" != "200" ]; then
    echo "❌ Game loading test failed: HTTP $response"
    exit 1
fi
echo "✅ Game loading works"
