...
description: Topología completa de la infraestructura AWS para RetroGameCloud incluyendo VPC, subnets, security groups y servicios distribuidos en múltiples AZ con flujos de tráfico detallados
...
graph TB
        ...
        subgraph VPC[🏢 VPC RetroGameCloud<br/>CIDR: 10.0.0.0/16]
                ...
                subgraph PRIV1[🔒 Private Subnet<br/>10.0.11.0/24]
                    subgraph EKS1[☸️ EKS Node Group 1<br/>t3.large instances]
                        subgraph PODS1[Pod Network]
                            KONG1[🦍 Kong Gateway<br/>API Gateway]
                            AUTH1[🔐 Auth Service<br/>JWT/OAuth]
                            GAME1[🎮 Game Service<br/>Emulation]
                            USER1[👤 User Service<br/>Profiles]
                            WS1[🔄 WebSocket Service<br/>Real-time]
                        end
                    end
                end
                
                subgraph DATA1[💾 Data Subnet<br/>10.0.21.0/24]
                    RDS1[(🗄️ RDS PostgreSQL<br/>Primary Instance<br/>Multi-AZ)]
                    REDIS1[(⚡ ElastiCache Redis<br/>Primary Node<br/>Cluster Mode)]
                end
            end
                ...
                subgraph PRIV2[🔒 Private Subnet<br/>10.0.12.0/24]
                    subgraph EKS2[☸️ EKS Node Group 2<br/>t3.large instances]
                        subgraph PODS2[Pod Network]
                            KONG2[🦍 Kong Gateway<br/>API Gateway]
                            AUTH2[🔐 Auth Service<br/>JWT/OAuth]
                            GAME2[🎮 Game Service<br/>Emulation]
                            USER2[👤 User Service<br/>Profiles]
                            WS2[🔄 WebSocket Service<br/>Real-time]
                        end
                    end
                end
                
                subgraph DATA2[💾 Data Subnet<br/>10.0.22.0/24]
            ...
            subgraph AZ3[📍 Availability Zone us-east-1c]
                ...
                subgraph PRIV3[🔒 Private Subnet<br/>10.0.13.0/24]
                    subgraph EKS3[☸️ EKS Node Group 3<br/>t3.medium instances]
            ...
            subgraph STORAGE[💽 Storage Layer]
                S3_GAMES[🪣 S3 Games Bucket<br/>Game ROMs & Assets]
                S3_SAVES[🪣 S3 Saves Bucket<br/>Game Save States]
                S3_STATIC[🪣 S3 Static Bucket<br/>Web Assets]
                S3_LOGS[🪣 S3 Logs Bucket<br/>Application Logs]
    ...
    subgraph EXTERNAL[🔌 External Services]
        ...
        SECRETS[🔒 AWS Secrets Manager<br/>API Keys & Passwords]
    end
