...
description: Topología completa de la infraestructura AWS para RetroGameCloud incluyendo
  VPC, subnets, security groups y servicios distribuidos en múltiples AZ con flujos
  de tráfico detallados
...
graph TB
    ...
    subgraph CloudFront[⚡ CloudFront Global CDN]
    ...
    subgraph Region[🌍 us-east-1 Virginia]
        subgraph VPC[🏢 VPC RetroGameCloud<br/>CIDR: 10.0.0.0/16]
            ...
            subgraph AZ1[📍 Availability Zone us-east-1a]
                subgraph PUB1[🌐 Public Subnet<br/>10.0.1.0/24]
                    ALB1[⚖️ Application Load Balancer<br/>Primary]
                    NAT1[🔄 NAT Gateway 1<br/>Elastic IP]
                    BASTION1[🔐 Bastion Host 1]
                end

                subgraph PRIV1[🔒 Private Subnet<br/>10.0.11.0/24]
                    subgraph EKS1[☸️ EKS Node Group 1<br/>t3.large instances]
                        subgraph PODS1[Pod Network]
                            API_POD1[🚀 API Gateway Pods<br/>3 replicas]
                            AUTH_POD1[🔐 Auth Service Pods<br/>2 replicas]
                            GAME_POD1[🎮 Game Service Pods<br/>3 replicas]
                            USER_POD1[👤 User Service Pods<br/>2 replicas]
                        end
                    end
                end

                subgraph DB1[🗄️ Database Subnet<br/>10.0.21.0/24]
                    RDS_PRIMARY[📊 RDS Primary<br/>PostgreSQL 14<br/>db.r5.large]
                    REDIS_PRIMARY[⚡ ElastiCache Primary<br/>cache.r6g.large]
                end
            end

            subgraph AZ2[📍 Availability Zone us-east-1b]
                subgraph PUB2[🌐 Public Subnet<br/>10.0.2.0/24]
                    ALB2[⚖️ Application Load Balancer<br/>Secondary]
                    NAT2[🔄 NAT Gateway 2<br/>Elastic IP]
                    BASTION2[🔐 Bastion Host 2]
                end

                subgraph PRIV2[🔒 Private Subnet<br/>10.0.12.0/24]
                    subgraph EKS2[☸️ EKS Node Group 2<br/>t3.large instances]
                        subgraph PODS2[Pod Network]
                            API_POD2[🚀 API Gateway Pods<br/>3 replicas]
                            AUTH_POD2[🔐 Auth Service Pods<br/>2 replicas]
                            GAME_POD2[🎮 Game Service Pods<br/>3 replicas]
                            USER_POD2[👤 User Service Pods<br/>2 replicas]
                        end
                    end
                end

                subgraph DB2[🗄️ Database Subnet<br/>10.0.22.0/24]
                    RDS_STANDBY[📊 RDS Standby<br/>PostgreSQL 14<br/>db.r5.large]
                    REDIS_REPLICA[⚡ ElastiCache Replica<br/>cache.r6g.large]
                end
            end

            subgraph AZ3[📍 Availability Zone us-east-1c]
                subgraph PUB3[🌐 Public Subnet<br/>10.0.3.0/24]
                    NAT3[🔄 NAT Gateway 3<br/>Elastic IP]
                end

                subgraph PRIV3[🔒 Private Subnet<br/>10.0.13.0/24]
                    subgraph EKS3[☸️ EKS Node Group 3<br/>t3.large instances]
                        subgraph PODS3[Pod Network]
                            API_POD3[🚀 API Gateway Pods<br/>2 replicas]
                            WORKER_POD3[⚙️ Worker Pods<br/>Background Jobs]
                            MONITOR_POD3[📊 Monitoring Pods<br/>Prometheus/Grafana]
                        end
                    end
                end

                subgraph DB3[🗄️ Database Subnet<br/>10.0.23.0/24]
                    REDIS_BACKUP[⚡ ElastiCache Backup<br/>cache.r6g.large]
                end
            end

            subgraph STORAGE[💾 Storage Layer]
                S3_ASSETS[🪣 S3 Games Assets<br/>Versioned/Encrypted]
                S3_BACKUPS[🪣 S3 Database Backups<br/>Cross-Region Replication]
                S3_LOGS[🪣 S3 Application Logs<br/>Lifecycle Policies]
                EFS[📁 EFS Shared Storage<br/>Cross-AZ Access]
            end

            subgraph CONTROL_PLANE[🎛️ EKS Control Plane]
                EKS_API[☸️ Kubernetes API Server]
                EKS_ETCD[💾 etcd Cluster]
                EKS_SCHEDULER[⚙️ Scheduler]
            end
...
graph TB
    subgraph VPC[VPC RetroGameCloud - 10.0.0.0/16]
        ...
        subgraph ROUTING[Route Tables]
            ...
            RT_PRIV1[Private RT AZ-1a<br/>0.0.0.0/0 → NAT-1]
            RT_PRIV2[Private RT AZ-1b<br/>0.0.0.0/0 → NAT-2]
            RT_PRIV3[Private RT AZ-1c<br/>0.0.0.0/0 → NAT-3]
