...
sequenceDiagram
    ...
    Note over U,M: Primera visita sin autenticación
    ...
    N-->>B: 302 Redirect → /oauth2/start
    
    Note over U,G: Proceso de autenticación GitHub
    
    B->>O: GET /oauth2/start
    O-->>B: 302 Redirect → GitHub OAuth
    ...
    G-->>B: Página de autorización
    U->>B: Autoriza aplicación
    
    Note over G,O: Intercambio de código por token
    
    G-->>B: 302 Redirect → /oauth2/callback?code=AUTHORIZATION_CODE
    ...
    Note over O: Validación de usuario y organización
    ...
    alt Usuario autorizado
        O-->>B: Set-Cookie: _oauth2_proxy=xxx + 302 Redirect → /
        Note over U,M: Acceso autorizado a la aplicación
        B->>N: GET / (con cookie)
        N->>O: auth-url: /oauth2/auth (con cookie)
        O->>O: Validar cookie/sesión
...
graph TB
    ...
    subgraph "Kubernetes Cluster"
        subgraph "Ingress Layer"
            I[🌐 Ingress NGINX<br/>grafana.retrogamehub.com]
        end
        
        subgraph "Auth Layer"
            O[🔐 OAuth2-Proxy<br/>Pod]
        end
    ...
    subgraph "External"
        ...
        ORG[🏢 retrogamecloud<br/>Organization]
    end
...
<AccordionGroup>
  <Accordion title="Error 403: Forbidden después de login">
    ...
    **Solución**:
    ...
    # Via GitHub UI: Settings → Member privileges
    ```
  </Accordion>
