...
description: "Guía completa para monitorizar y optimizar los costos operativos de RetroGameCloud en AWS"
icon: "coins"
---
...
<Warning>
Utiliza Spot Instances solo para workloads que toleran interrupciones, como procesamiento de métricas o tareas de backup.
</Warning>

**Configuración de Spot Node Group:**
...
**Política de Ciclo de Vida para Backups:**
...
<Tabs>
<Tab title="Comando de Análisis">
```bash
#!/bin/bash
# scripts/analyze-instance-utilization.sh

echo "=== Análisis de Utilización de Instancias EKS ==="

# CPU promedio por nodo en los últimos 7 días
kubectl top nodes

# Métricas detalladas de Prometheus
...
if [[ $(echo "$cpu_avg < 20" | bc) -eq 1 ]]; then
  echo "⚠️  CPU utilization < 20% - Consider t3.small instead of t3.medium"
fi
```
</Tab>
<Tab title="Configuración Optimizada">
```yaml
...
- name: optimized-workers
  # Cambio de t3.medium a t3.small basado en métricas
...
class EKSRightsizing:
    ...
    def get_node_utilization(self, instance_id, days=7):
        """Obtiene métricas de utilización de los últimos N días"""
    ...
    def recommend_instance_type(self, current_type, cpu_avg, memory_avg):
        """Recomienda tipo de instancia basado en utilización"""
        recommendations = {
            't3.medium': {
...
### Script de Detección y Limpieza
