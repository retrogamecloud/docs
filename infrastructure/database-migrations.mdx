...
Las migraciones de base de datos en RetroGameCloud requieren un enfoque estructurado para mantener la integridad de datos y minimizar el tiempo de inactividad. Esta guía cubre desde la creación de scripts hasta la ejecución en producción con estrategias de zero-downtime.
...
Para producción en AWS RDS PostgreSQL:
...
│   ├── R002__load_retro_games_catalog.sql
│   └── R003__default_user_preferences.sql
├── functions/
│   ├── V010__calculate_user_score.sql
│   └── V011__game_recommendation_engine.sql
└── procedures/
    ├── V020__backup_user_progress.sql
    └── V021__cleanup_expired_sessions.sql
```
...
<Tab title="Crear Índice">
```sql
-- V008__create_game_performance_indexes.sql
-- Índices para optimizar consultas de rendimiento de juegos
...
INSERT INTO game_categories (name, slug, description, icon, sort_order) VALUES
    ('Arcade', 'arcade', 'Juegos clásicos de arcade', 'joystick', 1),
    ('Puzzle', 'puzzle', 'Juegos de lógica y rompecabezas', 'puzzle-piece', 2),
    ('Platformer', 'platformer', 'Juegos de plataformas', 'running', 3),
    ('RPG', 'rpg', 'Juegos de rol clásicos', 'dragon', 4),
...
<Tab title="Fase 2: Migración">
```sql
-- V021__execute_user_avatar_migration.sql
-- Ejecutar migración en lotes para evitar locks largos
...
BEGIN
    ...
    RAISE NOTICE 'Iniciando migración de % avatares de usuario', total_users;
    ...
    RAISE NOTICE 'Migración completada: % avatares procesados', processed;
END
$$;
```
</Tab>
...
-- Solo ejecutar después de verificar que la aplicación funciona correctamente
...
BEGIN
    ...
    IF unmigrated_count > 0 THEN
        RAISE EXCEPTION 'Aún hay % usuarios sin migrar. Abortar limpieza.', unmigrated_count;
    END IF;
    
    RAISE NOTICE 'Verificación exitosa. Procediendo con limpieza.';
END
$$;
...
-- Crear vista temporal para compatibilidad durante transición
...
-- Programar eliminación de columna legacy (ejecutar en próximo release)
...
Para cambios críticos que requieren validación completa:
...
echo "🚨 ROLLBACK DE EMERGENCIA - Entorno: $ENVIRONMENT"
echo "🚨 Versión objetivo: $ROLLBACK_VERSION"

# Validaciones de seguridad
if [[ "$ENVIRONMENT" == "production" ]]; then
    echo "⚠️  ROLLBACK EN PRODUCCIÓN - Confirmación requerida"
...
echo "🔄 Ejecutando rollback a versión $ROLLBACK_VERSION..."
docker run --rm \
...
BEGIN
    ...
    IF array_length(missing_indexes, 1) > 0 THEN
        RAISE WARNING 'Índices faltantes: %', missing_indexes;
        result := FALSE;
    END IF;
...
BEGIN
    ...
    IF orphaned_scores > 0 THEN
        RAISE WARNING 'Scores huérfanos detectados: %', orphaned_scores;
        result := FALSE;
    END IF;
    
    -- Verificar usuarios inválidos
    ...
    IF invalid_users > 0 THEN
        RAISE WARNING 'Usuarios con datos inválidos: %', invalid_users;
        result := FALSE;
    END IF;
...
-- Test 3: Performance de consultas críticas
...
```bash
#!/bin/bash
# validate-migration.sh - Script de validación post-migración

set -e

echo "🔍 Iniciando validación de migración..."
...
Resources:
  ...
  DatabaseConnectionAlarm:
    ...
    Properties:
      ...
      AlarmDescription: 'Alerta por problemas de conexión durante migraciones'
...
<Warning>
Las migraciones de producción siempre deben ser ejecutadas durante ventanas de mantenimiento programadas y con equipos de respaldo disponibles.
</Warning>
...
- [ ] **Validación de rollback** preparada
- [ ] **Ventana de mantenimiento** programada
- [ ] **Equipo de respaldo** disponible
- [ ] **Monitoreo activo** configurado
- [ ] **Comunicación stakeholders** enviada
...
Esta guía consolidada proporciona un framework completo para manejar migraciones de base de datos de forma segura y eficiente en RetroGameCloud, desde el desarrollo hasta producción.