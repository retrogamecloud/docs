...
description: Guía completa para ejecutar migraciones de esquema en RDS PostgreSQL de forma segura, con estrategias de versionado, procedimientos de rollback y despliegues sin downtime
icon: database
---
...
│   └── U003__drop_game_scores_index.sql
├── data/
│   ├── V100__insert_initial_games.sql
│   └── V101__migrate_user_preferences.sql
└── hotfixes/
    └── V999__emergency_fix_user_scores.sql
```

## Ejemplos de Migraciones Reales

### Migración de Esquema Inicial
...
### Migración de Logros de Usuario
...
('Puntuación Alta', 'Consigue más de 10,000 puntos en cualquier juego', 25, 'score'),
('Veterano', 'Lleva 30 días registrado', 100, 'loyalty');
```
</Tab>
...
### Migración de Optimización de Índices
...
-- Índice parcial para juegos activos
CREATE INDEX CONCURRENTLY idx_games_active 
ON games(created_at) 
WHERE file_url IS NOT NULL;

-- Índice para ranking de puntuaciones
...
BEGIN
    ...
    RAISE NOTICE '✅ Integrity tests passed - Users: %, Games: %', user_count, game_count;
END $$;
```
</Tab>
</Tabs>
...
-- Verificar que el query use índices apropiados
DO $$
DECLARE
    query_plan TEXT;
BEGIN
...
<Tabs>
<Tab title="Preparación Blue-Green">
```bash
#!/bin/bash
...
echo "🔄 Iniciando migración Blue-Green..."
...
echo "⬆️ Ejecutando migraciones en Green..."
...
echo "🎯 Green environment listo para switch"
```
</Tab>
...
echo "📊 Monitoreando por 5 minutos..."
for i in {1..5}; do
    echo "Minuto $i/5..."
