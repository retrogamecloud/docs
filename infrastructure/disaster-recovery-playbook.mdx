...
```mermaid
graph TD
    A[Detección Incidente] --> B{Evaluar Severidad}
    B -->|P0-Crítico| C[Activar DR Inmediato]
    B -->|P1-Alto| D[Activar DR Planificado]
    B -->|P2-Medio| E[Procedimiento Estándar]

    C --> F[Notificar Stakeholders]
    D --> F
    E --> G[Monitoreo Continuo]

    F --> H[Ejecutar Runbook Específico]
    H --> I[Validar Recuperación]
    I --> J{Servicios OK?}
    J -->|Sí| K[Documentar Incident]
