---
title: "4.4. Servicio de Rankings"
...
El servicio de rankings proporciona tablas de clasificación (leaderboards) para cada juego, mostrando los mejores jugadores y sus puntuaciones.
...
Obtiene el ranking (top 10) de un juego específico.
...
- El campo `rank` se calcula automáticamente (1, 2, 3...)
...
**Query params:**
- `limit` (opcional): Número de jugadores a mostrar (default: 10, max: 100)

### GET /api/rankings/user/:username

Obtiene la posición de un usuario específico en todos los juegos.
...
### Query para Posición de Usuario
...
async function obtenerRankingPorJuego(gameSlug) {
  ...
  if (cached) {
    console.log('✅ Ranking desde cache');
  ...
  console.log('💾 Ranking guardado en cache');
...
async function invalidarCacheRanking(gameSlug) {
  ...
  console.log(`🗑️ Cache invalidado para: ${gameSlug}`);
}

// Llamar después de guardar score
...
**Mejora**: 16x más rápido con cache ✨
