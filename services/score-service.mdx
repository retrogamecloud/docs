---
title: "8.10. Servicio de Puntuaciones"
...
El servicio de puntuaciones permite a los jugadores guardar y actualizar sus mejores scores en cada juego. Solo guarda el score más alto por usuario y juego.
...
```json Response 200 (Actualización)
{
...
Obtiene todos los scores de un juego específico.
...
<Steps>
  <Step title="Validar autenticación">
    ```javascript
  ...
  <Step title="Guardar o actualizar">
    ...
    if (existing.rows.length > 0) {
      ...
      if (newScore > existing.rows[0].score) {
        ...
        return { message: 'Puntuación actualizada', score: newScore };
      } else {
        // Score existente es mejor → no actualizar
        return { message: 'Puntuación existente es mayor', score: existing.rows[0].score };
      }
    } else {
      // No existe → insertar
      await pool.query(
      ...
      return { message: 'Puntuación guardada', score: newScore };
    }
    ```
  </Step>
</Steps>
...
class ScoreTracker {
  async saveScore(game, score) {
    ...
    try {
      ...
      console.log('✅ Score guardado:', data.message);
...
<Warning>
  El **JWT_SECRET** debe ser el mismo que usa el Auth Service. Si no coincide, los tokens no se validarán correctamente.
</Warning>
