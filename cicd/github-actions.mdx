...
El pipeline de CI/CD de Retro Game Hub integra múltiples herramientas y servicios externos para automatización, seguridad, monitoreo y documentación:

<img src="/images/herramientas.png" alt="Herramientas e integraciones externas del proyecto" style={{width: '100%', maxWidth: '1000px', margin: '20px auto', display: 'block'}} />

*Figura 1: Stack de herramientas externas utilizadas en el proyecto: Snyk (seguridad), SonarCloud (calidad de código), DockerHub (registro de imágenes), Terraform Cloud (infraestructura), Mintlify (documentación) y Slack (notificaciones)*
...
## Documentación: Intelligent Docs Review

**Archivo**: `docs/.github/workflows/intelligent-docs-review.yml`

Workflow automatizado para revisar cambios en la documentación utilizando **GitHub Copilot** para análisis inteligente de calidad, consistencia y precisión técnica.
...
- Los cambios afectan archivos de documentación (`.mdx`, `.md`)
- Se modifican archivos de configuración (`docs.json`, `mint.json`)
...
#### 2. Análisis Inteligente con GitHub Copilot
...
<AccordionGroup>
  <Accordion title="Gramática y Estilo" icon="spell-check">
    - Ortografía y gramática en español
    - Consistencia en terminología técnica
    - Claridad y legibilidad del contenido
    - Uso correcto de mayúsculas y puntuación
  </Accordion>

  <Accordion title="Consistencia Técnica" icon="code">
    - Comandos y ejemplos de código funcionales
  ...
  <Accordion title="Estructura y Formato" icon="sitemap">
    - Formato MDX válido (frontmatter, componentes)
    - Jerarquía de headings correcta
    - Links internos y externos válidos
    - Bloques de código con sintaxis correcta
  </Accordion>

  <Accordion title="Precisión de Contenido" icon="bullseye">
...
El workflow genera comentarios automáticos en el PR con:

**Formato del comentario**:
```markdown
## 📚 Intelligent Docs Review
...
- **archivo.mdx:45** - Considerar añadir ejemplo de código
- **otro.mdx:23** - Verificar versión de dependencia mencionada

### 🔍 Observaciones Técnicas
- Detectado cambio en arquitectura (microservicios → backend unificado)
...
- ✅ **Success**: Documentación pasa todas las validaciones
- ⚠️ **Warning**: Sugerencias opcionales detectadas
- ❌ **Failure**: Errores críticos encontrados (sintaxis, links rotos)
...
```bash
# GitHub token automático (no requiere configuración)
GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
```

El token se proporciona automáticamente por GitHub Actions con permisos:
...
1. **Desarrollador crea PR** con cambios en documentación:
   ```bash
...
3. **Copilot comenta en el PR**:
   ...
   ⚠️ Línea 45: Considerar añadir leyenda al diagrama
   💡 Sugerencia: Mencionar alta disponibilidad multi-AZ
   ```

4. **Desarrollador responde**:
   - Aplica sugerencias
   - Push adicional → workflow re-ejecuta
   - Aprobación final → merge

### Beneficios

<Card title="Ventajas del Intelligent Docs Review" icon="star">
  - **Calidad Consistente**: Mantiene estándares en toda la documentación
  - **Detección Temprana**: Identifica errores antes del merge
  ...
  - **Documentación Actualizada**: Detecta información obsoleta automáticamente
</Card>

### Integración con Mintlify

El workflow valida específicamente:

```yaml
mintlify-specific:
  - Frontmatter válido (title, description, icon)
  - SidebarTitle con numeración correcta (1.1, 2.3, etc.)
  - Componentes Mintlify (<Card>, <Accordion>, <CodeGroup>)
  - Configuración docs.json/mint.json válida
...
Añadir `[skip review]` en el mensaje del commit omite el workflow.

### Métricas y Monitoreo

El workflow genera métricas accesibles en GitHub Actions:

- **Success Rate**: % de PRs que pasan sin errores
- **Average Review Time**: Tiempo promedio de análisis
- **Common Issues**: Tipos de errores más frecuentes
- **Files Modified**: Archivos más editados
