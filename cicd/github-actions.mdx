...
# Desde GitHub UI: Actions → Update Documentation → Run workflow
...
#### 2. Automático desde otros repos
...
<CardGroup cols={2}>
  <Card title="🚀 Documentación Always Up-to-Date" icon="check">
    La docs nunca se queda desactualizada porque se actualiza automáticamente con cada cambio
  </Card>
  ...
  <Card title="🔍 Cobertura Completa" icon="magnifying-glass">
...
## 🚀 Mejoras Inteligentes del Workflow (2025)

El workflow de análisis inteligente ha sido **potenciado con 5 nuevas capacidades automáticas** que lo hacen mucho más eficiente y completo:

### 1. ⚡ Auto-fix de Quick Wins

Aplica **automáticamente** correcciones simples sin necesidad de review humano:

<CardGroup cols={2}>
  <Card title="✨ Iconos en Frontmatter" icon="icons">
    Añade iconos apropiados a archivos que no los tienen
  </Card>

  <Card title="📝 Descripciones Generadas" icon="file-lines">
    Genera descriptions automáticas basadas en títulos
  </Card>

  <Card title="📐 Formato Estandarizado" icon="align-left">
...
<Steps>
  ...
  <Step title="Análisis Selectivo">
    Solo analiza archivos que cambiaron desde último análisis
  </Step>

  <Step title="Full Analysis Programado">
    Análisis completo automático 1x/semana
  </Step>
</Steps>
...
🔄 Análisis INCREMENTAL
   📝 Cambios por git: 3 archivos
   🔒 Cambios por hash: 2 archivos
   ⚡ Total a analizar: 5/50 archivos
   💰 Ahorro: 90% en tokens/costos
...
<Accordion title="Links Internos">
  ...
  - Valida anchors dentro de páginas
</Accordion>
...
<Accordion title="Imágenes y Assets">
  - Verifica que imágenes existan
...
- `api-reference/auth.mdx`: `/api/old-endpoint.mdx` → Archivo no encontrado

### 🌐 Links Externos (1)

- `infrastructure/overview.mdx`: `https://old-domain.com` → HTTP 404

### 🖼️ Imágenes (1)

- `quickstart.mdx`: `/images/screenshot-old.png` → Imagen no encontrada
...
<Tabs>
  <Tab title="Python">
    ```python
    # Detecta:
    - Funciones y clases públicas sin docstrings
...
<CardGroup cols={2}>
  ...
  <Card title="📊 Flow Diagrams" icon="diagram-next">
    Desde GitHub Actions workflows genera flujos CI/CD
  </Card>

  <Card title="📐 Component Diagrams" icon="cubes">
...
Los diagramas se **insertan automáticamente** en archivos `.mdx` relevantes.
...
# Desde GitHub UI
Actions → Daily Documentation Analysis → Run workflow
...
flowchart TD
    ...
    Links --> AST[3. Analizar Código AST]
    AST --> Diagrams[4. Generar Diagramas]
    Diagrams --> Cache[5. Check Cache]

    Cache -->|Full Needed| Analyze[6. Análisis Claude]
    Cache -->|Incremental| Skip[Skip Análisis]
...
<Note>
  Las siguientes capacidades están en el roadmap para 2025:
</Note>

1. **Métricas de Uso**: Integración con Mintlify analytics para priorizar mejoras
2. **GitHub Issues Automation**: Crear issues automáticamente para critical gaps
...
5. **Feedback Loop**: Integrar comentarios de PRs y issues en análisis
