...
<CardGroup cols={2}>
  <Card title="Tests Automáticos" icon="vial">
    - Unit tests con Jest
  ...
  <Card title="Rollback Automático" icon="arrow-rotate-left">
    - Health checks fallan → rollback
    - Smoke tests fallan → rollback
    ...
    - name: Rollback on failure
      ...
      run: |
        ...
        curl -X POST ${{ secrets.SLACK_WEBHOOK }} \
          -d '{"text":"🚨 Smoke tests failed. Rolling back deployment."}'
```
...
- **Security score**: Resultado del análisis de vulnerabilidades
