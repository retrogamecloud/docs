---
title: 7.1. Visión General de CI/CD
description: Pipeline completo de integración y despliegue continuo con GitHub Actions,
  Docker, ECR y ArgoCD con gates de aprobación y rollback automático
icon: rocket
---
...
graph TB
    ...
    subgraph CI["⚙️ CONTINUOUS INTEGRATION - GitHub Actions"]
        ...
        C3 --> C4[🛡️ Security Scan]
        C4 --> D{✅ All Tests Pass?}

        D -->|❌ Fail| STOP[🚫 Stop Pipeline<br/>Notify Developer]
        D -->|✅ Success| E[🏗️ Build Docker Image]

        E --> F[📦 Multi-stage Build]
        F --> G[🔒 Security Scan Image]
        G --> H[📤 Push to AWS ECR]
        H --> I[🏷️ Tag: latest, v1.0.X, sha-xxx]
    end

    subgraph GITOPS["📦 GITOPS WORKFLOW"]
        I --> J[📝 Update K8s Manifests]
        J --> K[⚙️ Kustomize Build]
    ...
    subgraph STAGING["🧪 STAGING DEPLOYMENT"]
        ...
        V -->|❌ Fail| ROLLBACK_STG[⏪ Rollback Staging]
        V -->|✅ Success| W[🎯 E2E Tests]

        W --> X{🧪 E2E Pass?}
        X -->|❌ Fail| ROLLBACK_STG
        X -->|✅ Success| Y[✅ Staging Validated]
    end

    subgraph APPROVAL["👥 APPROVAL GATE"]
        Y --> Z{🛡️ Manual Approval<br/>Required?}
        Z -->|No - Hotfix| AA[🚨 Auto-promote]
        Z -->|Yes| BB[👨‍💼 Approval Request]
        BB --> CC{✅ Approved?}
        CC -->|❌ Rejected| END1[⛔ Deployment Stopped]
        CC -->|✅ Approved| AA
    end

    subgraph PROD["🌟 PRODUCTION DEPLOYMENT"]
        AA --> DD[🎯 ArgoCD Sync to Prod]
        DD --> EE[🚀 Rolling Update Prod]
        EE --> FF[🏥 Health Check Prod]
        FF --> GG{💚 Production Healthy?}

        GG -->|❌ Fail| HH[🚨 Auto Rollback Prod]
        GG -->|✅ Success| II[📊 Post-deploy Monitoring]

        HH --> JJ[🔔 Alert Team]
        II --> KK{📈 Metrics OK?}
        KK -->|❌ Degraded| HH
        KK -->|✅ Success| LL[✅ Deployment Complete]
    end

    subgraph MONITORING["📊 CONTINUOUS MONITORING"]
        LL --> MM[📈 Application Metrics]
        MM --> NN[🏥 Health Checks]
        NN --> OO[📋 Log Aggregation]
        OO --> PP{🚨 Issues Detected?}
        PP -->|Yes| QQ[🔔 Alert & Consider Rollback]
        PP -->|No| RR[✅ Stable Production]
    end
