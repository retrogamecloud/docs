---
title: 6.1. Visión General de CI/CD
...
graph TB
    ...
    subgraph GITOPS["📦 GITOPS WORKFLOW"]
        ...
        P --> P1[✅ Approve & Merge]
        N -->|Yes| P1
        P1 --> Q[📁 Config Repo Updated]
    end

    subgraph CD["🚀 CONTINUOUS DEPLOYMENT - ArgoCD"]
        Q --> R[🔍 ArgoCD Detects Changes]
        R --> S[📥 Sync Application]
        S --> T[🔄 Rolling Update]
        T --> U[🏥 Health Checks]

        U --> V{💚 Healthy?}
        V -->|❌ Fail| W[⏪ Automatic Rollback]
        V -->|✅ Success| X[✅ Deployment Complete]

        W --> W1[📧 Alert Operators]
        X --> X1[📊 Update Metrics]
    end

    subgraph MONITOR["📊 MONITORING"]
        X1 --> Y[📈 Prometheus Metrics]
        Y --> Z[📊 Grafana Dashboard]
        Z --> Z1[🔔 Slack Notifications]
    end
