---
title: '7. Diagramas de Secuencia'
...
sequenceDiagram
    ...
    Frontend->>Frontend: Validación cliente<br/>(username 3-20 chars,<br/>email válido, password 6+)
    ...
    AuthService->>AuthService: Validación servidor
...
sequenceDiagram
    ...
    alt Usuario no existe
        ...
        Kong-->>Frontend: Error: Credenciales inválidas
        ...
        alt Password incorrecta
            ...
            AuthService->>Redis: SET session:userId<br/>Guardar sesión<br/>TTL: 24h
            Redis-->>AuthService: OK
...
sequenceDiagram
    ...
    alt Token no presente
        ...
        alt Token inválido o expirado
            AuthService-->>Kong: 401 Unauthorized
            Kong-->>Frontend: Error: Token inválido
            ...
            alt Sesión no existe
                ...
                AuthService-->>Kong: 401 Unauthorized
                Kong-->>Frontend: Error: Sesión expirada
            else Sesión válida
                Redis-->>AuthService: Datos de sesión
                
                AuthService->>Redis: EXPIRE session:userId 86400<br/>Renovar TTL
                Redis-->>AuthService: OK
                
                AuthService-->>Kong: 200 OK<br/>{valid: true, user}
                Kong-->>Frontend: Token válido
                Frontend->>Frontend: Mantener sesión activa
            end
        end
    end
```
...
sequenceDiagram
    ...
    alt Rate limit excedido
        ...
        alt Validación falla
            ...
            ScoreTracker-->>Usuario: Score inválido
            ...
            alt Nuevo score <= Score anterior
                ...
                ScoreTracker-->>Usuario: No superaste tu récord
                ...
                ScoreTracker-->>Usuario: ¡Nuevo récord!<br/>Score: 50,000
            end
        end
    end
```
...
sequenceDiagram
    ...
    alt Cache Hit
        ...
        Frontend->>Frontend: Resaltar top 3<br/>(🥇🥈🥉)
        Frontend->>Frontend: Resaltar usuario actual<br/>(si está en top 100)
        Frontend-->>Usuario: Mostrar rankings
    end
...
sequenceDiagram
    ...
    Usuario->>Frontend: Click en juego "DOOM"
    Usuario->>Frontend: Seleccionar período "monthly"
    
    Frontend->>Kong: GET /rankings/game/doom?period=monthly
    
    Kong->>RankingService: Forward request
    
    RankingService->>RankingService: Validar período<br/>(alltime|monthly|weekly|daily)
    ...
    alt Cache Hit
        ...
        RankingService->>RankingService: Calcular fecha inicio<br/>monthly: 1er día del mes<br/>weekly: hace 7 días<br/>daily: hoy 00:00
        ...
        DB-->>RankingService: Scores del período
    ...
    Frontend->>Frontend: Buscar posición usuario actual
    
    alt Usuario en top 100
        Frontend->>Frontend: Resaltar fila<br/>del usuario
        Frontend-->>Usuario: "Estás en posición #15"
        ...
        DB-->>RankingService: Posición calculada
        RankingService-->>Frontend: {position: 245}
        Frontend-->>Usuario: "Estás en posición #245"
    end
...
sequenceDiagram
    ...
    Note over Redis: Próximo request regenerará cache
```
...
stateDiagram-v2
    ...
    Degraded --> Critical: Múltiples pods fallan
    ...
    Critical --> Recovering: Intervención manual
...
sequenceDiagram
    ...
    alt Tests fail
        GitHubActions-->>Developer: ❌ Build failed<br/>(notification)
    else Tests pass
        ...
        GitHubActions-->>Developer: ✅ Build successful
        ...
        Kubernetes->>Kubernetes: Rolling update<br/>(old pod → new pod)
        ...
        alt Health checks pass
            ...
            ArgoCD-->>Developer: 🚀 Deployed to production
        else Health checks fail
            Kubernetes->>Kubernetes: Rollback automático
            Kubernetes-->>ArgoCD: Sync failed
            ArgoCD-->>Developer: ⚠️ Deployment failed<br/>Rolled back
        end
    end
```
