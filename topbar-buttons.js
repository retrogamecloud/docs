...
(function() {
  function addCustomButtons() {
    ...
    // Mobile: find the parent container that has the search button
    ...
    if (document.getElementById('custom-github-btn') || document.getElementById('custom-github-btn-mobile')) {
      return;
    }
    ...
    function createButtons(isMobile = false) {
      ...
      if (isMobile) {
        // Mobile: icon only - ensure SVG is visible
        githubBtn.className = 'text-gray-500 w-7 h-7 flex items-center justify-center hover:text-gray-600 dark:text-gray-400 dark:hover:text-gray-300';
        githubBtn.innerHTML = `
    ...
    if (mobileContainer && searchBarMobile) {
      ...
      mobileButtons.className = 'flex items-center gap-2 mr-2'; // More spacing
