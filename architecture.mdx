...
description: Visión general de la arquitectura de microservicios cloud-native de Retro
  Game Hub desplegada en AWS EKS, incluyendo decisiones arquitectónicas, infraestructura
...
graph TB
    ...
    subgraph "AWS GLOBAL"
        subgraph "Route 53"
            DNS[🌐 Route 53<br/>DNS Resolution<br/>retrogamehub.com]
        end

        subgraph "CloudFront Global"
            CDN[📡 CloudFront CDN<br/>Edge Locations<br/>Static Assets & Game Files]
        end
    end
        ...
        subgraph "VPC: 10.0.0.0/16"
            subgraph "Public Subnets"
                subgraph "AZ-1a: 10.0.1.0/24"
                    ALB1[⚖️ Application Load Balancer<br/>SSL/TLS Termination<br/>Target Groups]
            ...
            subgraph "Private Subnets - Application Tier"
                subgraph "App-AZ-1a: 10.0.10.0/24"
                    EKS1[☸️ EKS Worker Nodes<br/>Application Pods<br/>c5.large instances]
                end

                subgraph "App-AZ-1b: 10.0.20.0/24"
                    EKS2[☸️ EKS Worker Nodes<br/>Application Pods<br/>c5.large instances]
                end

                subgraph "App-AZ-1c: 10.0.30.0/24"
                    EKS3[☸️ EKS Worker Nodes<br/>Application Pods<br/>c5.large instances]
                end
            end

            subgraph "Private Subnets - Data Tier"
                subgraph "DB-AZ-1a: 10.0.100.0/24"
                    RDS1[(🗄️ RDS PostgreSQL<br/>Primary Instance<br/>db.t3.medium)]
                    REDIS1[(⚡ ElastiCache Redis<br/>Primary Node<br/>cache.t3.micro)]
                end

                subgraph "DB-AZ-1b: 10.0.200.0/24"
                    RDS2[(🗄️ RDS PostgreSQL<br/>Read Replica<br/>db.t3.medium)]
                    REDIS2[(⚡ ElastiCache Redis<br/>Replica Node<br/>cache.t3.micro)]
                end
            end
        end

        subgraph "S3 Storage"
            S3Games[🗂️ S3 Bucket<br/>retro-games-storage<br/>ROM Files & Assets]
            S3Static[🗂️ S3 Bucket<br/>retro-static-assets<br/>Frontend Static Files]
            S3Logs[🗂️ S3 Bucket<br/>retro-logs<br/>Application Logs]
        end
...
graph TB
    subgraph "API Gateway Layer"
        APIGW[🚪 Kong API Gateway<br/>Rate Limiting, Auth, Routing<br/>Namespace: api-gateway]
    end

    subgraph "Core Business Services"
        USER[👤 User Service<br/>Authentication & Profiles<br/>Port: 8001<br/>Namespace: user-service]

        GAME[🎮 Game Service<br/>Game Catalog & Metadata<br/>Port: 8002<br/>Namespace: game-service]

        EMULATOR[🖥️ Emulator Service<br/>Game Execution Engine<br/>Port: 8003<br/>Namespace: emulator-service]

        COLLECTION[📚 Collection Service<br/>User Game Collections<br/>Port: 8004<br/>Namespace: collection-service]

        ACHIEVEMENT[🏆 Achievement Service<br/>Gaming Achievements<br/>Port: 8005<br/>Namespace: achievement-service]

        SOCIAL[👥 Social Service<br/>Friends & Social Features<br/>Port: 8006<br/>Namespace: social-service]

        NOTIFICATION[📨 Notification Service<br/>Push & Email Notifications<br/>Port: 8007<br/>Namespace: notification-service]
    end

    subgraph "Supporting Services"
        FILE[📁 File Service<br/>ROM & Asset Management<br/>Port: 8008<br/>Namespace: file-service]

        ANALYTICS[📊 Analytics Service<br/>Usage & Performance Metrics<br/>Port: 8009<br/>Namespace: analytics-service]

        SEARCH[🔍 Search Service<br/>Game Search & Discovery<br/>Port: 8010<br/>Namespace: search-service]
    end

    subgraph "Frontend Applications"
        WEB[🌐 Web Frontend<br/>React SPA<br/>retrogamehub.com]

        MOBILE[📱 Mobile App<br/>React Native<br/>iOS & Android]

        ADMIN[⚙️ Admin Panel<br/>Vue.js Dashboard<br/>admin.retrogamehub.com]
    end
...
- **Search Engine**: Elasticsearch 7.17 para búsqueda de juegos
...
- **Container Runtime**: Docker con imágenes multi-stage
...
- **Escalabilidad horizontal**: Cada servicio escala según su demanda
...
- **Escalabilidad automática**: HPA y VPA para escalar pods según demanda
...
- **Desarrollo rápido**: TypeScript para type safety manteniendo productividad
...
- **Full-text search**: Capacidades de búsqueda nativas
...
### 6. S3 + CloudFront para Assets Estáticos
...
- **Cost effectiveness**: S3 storage classes para optimizar costos según uso
...
- **Network ACLs**: Control de tráfico a nivel de subnet
