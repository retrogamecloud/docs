...
Obtiene el ranking combinado de los mejores jugadores en todos los juegos. Se calcula basándose en el promedio de posiciones de cada jugador.
  ...
  <Expandable title="Ranking Object">
    ...
    <ResponseField name="best_game" type="object">
      ...
      <Expandable title="properties">
        ...
        <ResponseField name="rank" type="number">
          Posición en ese juego
        </ResponseField>

        <ResponseField name="score" type="number">
          Puntuación en ese juego
        </ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
...
<ResponseField name="cached_at" type="string">
  Timestamp de cuándo se generó el caché (ISO 8601)
</ResponseField>
...
async function loadGlobalLeaderboard() {
  ...
  console.log(`Última actualización: ${data.cached_at}`);
}
...
El ranking global se calcula usando la siguiente fórmula:
...
function calculateGlobalRank(player) {
  const weights = {
    ...
    totalScore: 0.3,       // 30% - Puntuación total acumulada
...
<Info>
  El ranking global favorece la **consistencia** (buenos rankings en múltiples juegos) sobre puntuaciones altas en un solo juego.
</Info>
...
- **Recomendación**: Cachea el resultado en el frontend y refresca cada 5 minutos

## Comparación con Tu Ranking

```javascript
// Mostrar posición del usuario actual en el ranking global
...
async function showMyPosition(userId) {
  ...
  if (myPosition) {
    console.log(`Estás en el puesto #${myPosition.rank} del ranking global!`);
  } else {
    // Si no está en top 100, obtener scores del usuario
