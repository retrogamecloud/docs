---
title: "5.2. GET /rankings/game/:gameSlug"
description: "Obtener ranking de un juego específico"
...
Obtiene la tabla de clasificación (leaderboard) de un juego específico, ordenada por puntuación más alta.
...
<ParamField query="limit" type="number" default="50">
  Número de jugadores a retornar (1-100)
</ParamField>
...
<ResponseField name="rankings" type="array">
  Array de jugadores ordenados por puntuación
...
<Tabs>
  <Tab title="All-Time">
    **Todos los tiempos**
    ...
    Ranking histórico con todas las puntuaciones registradas desde el inicio.
  ...
  <Tab title="Monthly">
    **Mes actual**
    ...
    Solo puntuaciones del mes en curso (se resetea el día 1).
  ...
  <Tab title="Weekly">
    **Semana actual**
    ...
    Solo puntuaciones de los últimos 7 días.
    
    - Caché: 15 minutos
    - Uso: Desafíos semanales
  </Tab>
  
  <Tab title="Daily">
    **Hoy**
    
    Solo puntuaciones del día actual.
...
```javascript
// Game Leaderboard Component con múltiples periodos
...
class GameLeaderboard {
  ...
  renderTopThree() {
    ...
    return `
      <div class="podium">
        ${top3[1] ? this.renderPodiumPlayer(top3[1], 2, '🥈') : ''}
        ${top3[0] ? this.renderPodiumPlayer(top3[0], 1, '🥇') : ''}
        ${top3[2] ? this.renderPodiumPlayer(top3[2], 3, '🥉') : ''}
      </div>
    `;
  }
    ...
    if (myRank) {
      return `
        <div class="my-rank highlight">
          <strong>Tu posición:</strong> #${myRank.rank}
      ...
      return `
        <div class="my-rank">
          <p>Aún no tienes puntuación en este juego</p>
...
<Info>
  Los rankings se cachean en **Redis con diferentes TTL** según el periodo:
...
Cuando un jugador envía una nueva puntuación, el sistema:

1. Guarda el score en PostgreSQL
2. Publica evento a Redis Pub/Sub
3. Ranking Service escucha el evento
4. Invalida caché del juego afectado
5. Próxima petición regenera el ranking

```javascript
// Ejemplo de invalidación (backend)
...
- **Recomendación**: Cachea en el frontend y refresca cada 2-5 minutos
...
async function showMonthlyCompetition(gameSlug) {
  ...
  console.log(`🏆 Competencia de ${data.game.name} - ${daysLeft} días restantes`);
