---
title: 8.2. Solución de Problemas Comunes
description: Guía de resolución de problemas comunes en Retro Game Hub
icon: file-lines
---

# 🔧 Troubleshooting

Guía completa para diagnosticar y resolver problemas comunes en Retro Game Hub.
...
- Pantalla negra después de seleccionar juego
...
# 2. Verificar permisos públicos del bucket
...
# Aplicar política CORS correcta
cat > cors.json <<EOF
{
  "CORSRules": [{
...
// Si es undefined, verificar que js-dos.js está cargado
...
# 4. Verificar JWT_SECRET está configurado
...
# Verificar Security Group permite tráfico desde EKS
...
- Score no cambia después de jugar

#### Diagnóstico

```bash

# 1. Verificar score-service está corriendo
kubectl get pods -n backend -l app=score-service

# 2. Ver logs del servicio
kubectl logs -f deployment/score-service -n backend --tail=100

# 3. Verificar Redis está accesible
...
# Ver configuración actual en Kong
...
* *B. Validación de score falla (anti-cheat)**

```javascript
// Verificar rangos válidos en score-service
...
// Si el score está fuera de rango, ajustar en variables de entorno
...
# Los rankings se regenerarán en el siguiente request
...
# Si no hay logs, verificar configuración de Redis Pub/Sub
...
# - Configuración inválida en kong.yml
...
# Si configuración es inválida, corregir y aplicar
...
# Si está en Pending, verificar:

# - Límite de ELBs en cuenta AWS
...
# 3. Verificar última modificación en S3
...
```html
<!-- Agregar query string para forzar actualización -->
...
* *A. Error de conexión a base de datos**
...
- Requests tardan más de 2 segundos
...
```javascript
// Aumentar pool size en configuración
const pool = new Pool({
...
- [ ] Las métricas de uso de recursos

- [ ] Los logs de Kong API Gateway

- --

<Card title="¿Problema No Resuelto?" icon="life-ring">
  Si no encuentras solución aquí, consulta los [diagramas de secuencia](/sequence-diagrams) para entender mejor el flujo del sistema, o abre un issue en GitHub.
</Card>
