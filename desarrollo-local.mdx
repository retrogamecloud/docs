---
title: "3. Desarrollo Local"
description: "Ejecuta RetroGame Cloud en tu máquina local paso a paso"
icon: "laptop-code"
---
...
<Warning>
  Ejecutar múltiples terminales es tedioso - por eso usamos Docker 👇
</Warning>
...
<Tip>
  Docker resuelve el problema de "funciona en mi máquina" - pero gestionar 6+ contenedores manualmente es complejo. Siguiente nivel: Docker Compose 👇
</Tip>
...
<Steps>
  <Step title="Levantar todos los servicios">
    ...
    Verás:
    ```
    ✅ Network retrogame_default created
    ✅ Volume postgres_data created
    ✅ Volume redis_data created
    🐘 postgres started
    🔴 redis started
    🔐 auth-service started
    🎮 game-catalog-service started
    🏆 score-service started
    📊 ranking-service started
  ...
  <Step title="Probar la aplicación">
    ```bash
    # Frontend
...
<Tip>
  Docker Compose es perfecto para desarrollo local - pero para producción necesitamos orquestación real. Siguiente nivel: Kubernetes 👇
</Tip>
...
Minikube crea un cluster de Kubernetes en tu máquina para probar despliegues.
...
<Steps>
  ...
  <Step title="Usar registro de imágenes local">
    ```bash
    # Apuntar Docker al daemon de Minikube
    eval $(minikube docker-env)
    
    # Ahora buildea las imágenes
...
<Table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Desarrollo rápido</td>
      <td>Testing individual</td>
      <td>Desarrollo completo</td>
      <td>Testing pre-producción</td>
    </tr>
  </tbody>
</Table>
...
<CardGroup cols={2}>
  <Card title="Desplegar en AWS EKS" icon="aws" href="/quickstart">
    Lleva tu aplicación a producción
  </Card>
