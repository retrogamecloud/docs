...
<Warning>
  **Antes de comenzar**: Completa el [Bootstrap Inicial](/infrastructure/bootstrap) para configurar credenciales AWS y backend de Terraform.
</Warning>
...
<Checklist>
  ...
  - [ ] Usuario IAM con permisos suficientes (Admin o política custom)
  - [ ] Límites de servicio verificados (Service Quotas para EKS, VPC)
  - [ ] Backend de Terraform creado (S3 + DynamoDB)
  - [ ] Repositorio clonado: `git clone https://github.com/retrogamecloud/infrastructure.git`
  - [ ] `terraform.tfvars` configurado con valores reales
  - [ ] Backend configurado en `main.tf`
  - [ ] GitHub OAuth App creada (para monitoreo)
  ...
  - [ ] Herramientas instaladas: terraform >= 1.5, kubectl >= 1.28, aws-cli >= 2.x
</Checklist>

<Tip>
  Si algún item falla, consulta la [Guía de Bootstrap](/infrastructure/bootstrap) completa.
</Tip>
...
<Steps>
  ...
  <Step title="Configurar kubectl y Verificar">
    ...
    <Tip>
      Si algún pod está en `Pending` o `CrashLoopBackOff`, verifica los logs:
      ```bash
...
<CardGroup cols={2}>
  ...
  <Card title="ArgoCD" icon="arrows-rotate" href="https://retrogamehub.games/argocd">
    ```
...
<Note>
  Los servicios de monitoreo (Grafana, Prometheus, ArgoCD) están protegidos por **GitHub OAuth** - solo usuarios autorizados pueden acceder.
</Note>
